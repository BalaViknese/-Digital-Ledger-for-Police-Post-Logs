...
ORDER BY count DESC;""",
    "Country Arrest Stats": """SELECT YEAR(stop_date) AS year, country_name,
                                        COUNT(*) AS total_stops,
    ...
    "Stop Duration by Age and Violation": """SELECT driver_age, violation, AVG(stop_duration) AS avg_duration
                                             FROM stops
...
GROUP BY violation
HAVING 
    SUM(CASE WHEN search_conducted = 'True' THEN 1 ELSE 0 END) > 10
    AND 
    SUM(CASE WHEN is_arrested = 'True' THEN 1 ELSE 0 END) > 5;""",
    ...
    "Top 5 Violations with Highest Arrest Rate": """SELECT violation,
       SUM(CASE WHEN is_arrested = 'True' THEN 1 ELSE 0 END) / COUNT(*) AS arrest_rate
FROM stops
GROUP BY violation
